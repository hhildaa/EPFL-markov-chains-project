...
if __name__ == '__main__':
    ...
    for i in range(sim_num):
        ...
        final_state, acceptance_rate_list, estimation_overlaps = \
            baseline(start, adj_matrix, 1, x_star, it_num=it_num)

        overlap = assess_estimation_quality(final_state, x_star)
        average_over_runs.append(overlap)
        acceptances_over_runs.append(acceptance_rate_list)

        plot_estimation_overlaps_over_iterations(estimation_overlaps)
